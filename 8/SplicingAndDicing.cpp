...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* pdna = nullptr;
    Nucleotide* start = new Nucleotide;
    // Process first char
    start -> prev = nullptr;
    start -> value = str[0];
    pdna = start;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    // When only one left
    if (dna->value == tof ->value && tof->next == nullptr) {
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    auto index = findFirst(dna,target);
    if (index == nullptr && dna != nullptr) {
        return false;
    } else if (target == nullptr) {
        return true;
    } else if (dna -> prev == nullptr && dna -> next == nullptr) {
        delete dna;
        return true;
    }
    Nucleotide* curr = index;
