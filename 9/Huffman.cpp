...
using namespace std;
PriorityQueue<pair<EncodingTreeNode*, int>> sweight(const string& ipt) {
    map<char, int> freq;
    for (auto c: ipt) {
    ...
    for (auto Pair: freq) {
        ...
        node->zero = nullptr;
        node->one = nullptr;
        pq.enqueue({node, Pair.second}, Pair.second);
    }
    return pq;
}
...
void traversing(EncodingTreeNode* node, string prefix, map<char, string>& table) {
    if (isleaf(node)) {
        table[node->ch] = prefix;
    } else {
    if (node -> zero != nullptr) {
        traversing(node->zero, prefix+"0", table);
    }
    if (node -> one != nullptr) {
        traversing(node->one, prefix+"1", table);
    }
}}

void decodeHelper(EncodingTreeNode* node, Queue<Bit>& bits, Queue<char>& leaves) {
    if (!bits.isEmpty()) {
        if (bits.dequeue() == 0) {
            node->ch = leaves.dequeue();
            node->zero = node->one = nullptr;
        } else {
            node -> zero = new EncodingTreeNode;
            node -> one = new EncodingTreeNode;
            decodeHelper(node->zero,bits,leaves);
            decodeHelper(node->one,bits,leaves);
        }
    }
}
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    PriorityQueue<pair<EncodingTreeNode*, int>> pq = sweight(str);
    while(pq.size() >= 2) {
        auto t1 = pq.dequeue();
        auto t2 = pq.dequeue();
        EncodingTreeNode* node = new EncodingTreeNode;
...
HuffmanResult compress(const string& text) {
    auto Hufftree = huffmanTreeFor(text);
    Queue<Bit> tbits;
    Queue<char> leaves;
    Queue<Bit> message = encodeText(text, Hufftree);
    encodeTree(Hufftree, tbits, leaves);
    deleteTree(Hufftree); // Required, or leading to leak.
    return {tbits, leaves, message};
}
...
string decompress(HuffmanResult& file) {
    auto Hufftree = decodeTree(file.treeBits, file.treeLeaves);
    auto ans = decodeText(file.messageBits, Hufftree);
    deleteTree(Hufftree);
    return ans;
}
