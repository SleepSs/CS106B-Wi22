...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    slotNums = hashFn.numSlots();
    elems = new Slot[slotNums]();
    logicalSize = 0;
    this -> hashFn = hashFn;
...
bool RobinHoodHashTable::insert(const string& elem) {
    int iii = where(elem);
    if (iii != -1 || slotNums == logicalSize) return false;
    int hashv = hashFn(elem);
    if (elems[hashv].distance == -1) {
        elems[hashv].value = elem;
        elems[hashv].distance = 0;
        logicalSize++;
        return true;
    } else {
        int index = 1;
        while (hashv+index < slotNums) {
            if (elems[index+hashv].distance == -1) {
                elems[index+hashv].value = elem;
                elems[index+hashv].distance = index;
                logicalSize++;
                return true;
            } else if (elems[index+hashv].distance < index){
                string old = elems[index+hashv].value;
                elems[index+hashv].value = elem;
                elems[index+hashv].distance = index;
                // Don't do logicalSize
                return insert(old);
            }
            index++;
        }
        int j = 0;
        while (true) {
            if (elems[j].distance == -1) {
                elems[j].value = elem;
                elems[j].distance = index;
                logicalSize++;
                return true;
            } else if (elems[j].distance < index ){
                string old = elems[j].value;
                elems[j].value = elem;
                elems[j].distance = index;
                // Don't do logicalSize
                return insert(old);
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (index+1 < slotNums && elems[index+1].distance > 0) {
    ...
    logicalSize--;
    int j = 0;
    if (index+1 < slotNums || elems[j].distance <= 0) {
        elems[index].distance = -1; // Remove myself
        return true;
    }
    elems[index] = elems[j];
    elems[index].distance--;
    while (elems[j+1].distance >0 ) {
        elems[j] = elems[j+1];
        elems[j].distance--;
        j++;
    }
