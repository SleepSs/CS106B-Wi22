...
bool LinearProbingHashTable::insert(const string& elem) {
    if (logicalSize == slotNums) {
        return false;
    } else {
        auto hashv = hashFn(elem);
        if (elems[hashv].type == SlotType::FILLED) {
            if (elems[hashv].value == elem) {
                return false;
            } else {
                if (linearSearch(elem, hashv) == -1) {
                   return linearInsert(elem, hashv);
                } else {
                    return false;
                }
            }
            ...
            elems[hashv].type = SlotType::FILLED;
            logicalSize++;
            return true;
        }
    }
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    if (elems[hashv].type == SlotType::FILLED) {
        if (elems[hashv].value == elem) {
            ...
            return (linearSearch(elem, hashv) != -1);
        }
    } else if (elems[hashv].type == SlotType::TOMBSTONE) {
        return (linearSearch(elem, hashv) != -1);
    }
    return false;
...
bool LinearProbingHashTable::remove(const string& elem) {
    int hashv = hashFn(elem);
    if (elems[hashv].value == elem && elems[hashv].type == SlotType::FILLED) { // Logic not complete
        elems[hashv].type = SlotType::TOMBSTONE;
        logicalSize--;
        return true;
    } else {
        int index = linearSearch(elem, hashv);
        if (index != -1) {
