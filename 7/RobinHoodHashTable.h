...
private:
    ...
    int where(std::string elem) const {
        int hashv = hashFn(elem);
        if (elems[hashv].value == elem && elems[hashv].distance != -1) {
            return hashv;
        } else {
            int index = 1;
            while (hashv+index < slotNums && elems[hashv+index].distance >= index) {
                if (elems[hashv+index].value == elem) {
                    return hashv+index;
                }
                index++;
            }
            if (hashv+index < slotNums) {
                return -1;
            } else {
                int j = 0;
                while (elems[j].distance >= index) {
                    if (elems[j].value == elem) {
                        return j;
                    }
                    j++;
                }
