...
private:
    ...
    int linearSearch(const std::string& elem, int index) const{
        for (int i = 1;i + index < slotNums; i++) {
            if (elems[i+index].type == SlotType::EMPTY) {
                return -1;
            }
            if (elems[i+index].value == elem && elems[i+index].type == SlotType::FILLED) {
                return i + index;
            }
        }
        int j = 0;
        while (true) {
            if (elems[j].type == SlotType::EMPTY) {
                return -1;
            } else if (elems[j].value == elem && elems[j].type == SlotType::FILLED) {
                return j;
            }
            j++;
        }
    }
    bool linearInsert(const std::string& elem, int index) {
        for (int i = 1;i+index < slotNums; i++) {
            if (elems[i+index].type != SlotType::FILLED) {
                elems[i+index].value = elem;
                elems[i+index].type = SlotType::FILLED;
                logicalSize++;
                return true;
            }
        }
        int j = 0;
        while (true) {
            if (elems[j].type != SlotType::FILLED) {
                elems[j].value = elem;
                elems[j].type = SlotType::FILLED;
                logicalSize++;
                return true;
            }
            j++;
