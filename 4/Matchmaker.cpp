...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    /* Don't fear const, make as much auxiliary var as you want */
    if (possibleLinks.isEmpty()) {
        return true;
    } else {
        string guy = possibleLinks.firstKey();
        if (possibleLinks[guy].isEmpty()) {
            return false;
        } else {
            for (const string &aguy: possibleLinks[guy]) {
                if (possibleLinks.containsKey(aguy)) {
                    Map<string, Set<string>> bklink = possibleLinks;
                    bklink.remove(guy);
                    bklink.remove(aguy);
                    if (hasPerfectMatching(bklink, matching)) {
                        matching.add({guy, aguy});
                        return true;
                    }
                }
            }
...
int whelper(Map<string, Map<string, int>>& pl, int score, Set<Pair>& chosen) {
    if (pl.isEmpty()) {
        return score;
    } else {
        string guy = pl.firstKey();
        if (pl[guy].isEmpty()) {
            pl.remove(guy);
            return whelper(pl, score, chosen);
        } else {
            string aguy=pl[guy].keys()[0];
            if (pl.containsKey(aguy) && pl[guy][aguy] > 0) {
                Map<string, Map<string, int>> bklink = pl;
                auto bkchosen = chosen;
                bklink.remove(guy);
                bklink.remove(aguy);
                auto c1 = whelper(bklink, score+pl[guy][aguy], chosen += {{guy, aguy}});
                Map<string, Map<string, int>> bblink = pl;
                bblink[guy].remove(aguy);
                auto c2 = whelper(bblink, score, bkchosen);
                if (c1 >= c2) {
                    return c1;
                } else {
                    chosen = bkchosen;
