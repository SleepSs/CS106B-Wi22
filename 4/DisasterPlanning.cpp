...
Set<string> Allcity(const Map<string, Set<string>>& roadNetwork) {
    Set<string> citii;
    for(auto x: roadNetwork.keys()) {
        citii.add(x);
    }
    return citii;
}
Set<string> SecureAll(const Map<string, Set<string>>& roadNetwork, string city) {
    Set<string> saved;
    saved += {city};
    for (auto x: roadNetwork[city]) {
        saved.add(x);
    }
    return saved;
}
bool helper(const Map<string, Set<string>>& roadNetwork,
            int numCities,
            Set<string>& supplyLocations, Set<string>& InSecured) {
    if (numCities < 0) {
        error("negative cities");
    } else if (InSecured.size() == 0){
        return true;
    } else if (numCities == 0) {
        return false;
    } else {
        string city = InSecured.first();
        Set<string> copyin = InSecured;
        Set<string> copysu = supplyLocations;
        // Attempt to put stockpile in current city
        auto sc = SecureAll(roadNetwork, city);
        supplyLocations.add(city);
        if(helper(roadNetwork, numCities-1, supplyLocations, InSecured -= sc)) {
            return true;
        }
        // Try neighbouring guy
        for (auto ncity: roadNetwork[city]) {
            // Don't use auto supplyLocations = ..., cause it will create a new s..L instead the original & one
            sc = SecureAll(roadNetwork, ncity);
            supplyLocations = copysu;
            InSecured = copyin;
            supplyLocations.add(ncity);
            if(helper(roadNetwork, numCities-1, supplyLocations, InSecured -= sc)) {
                return true;
            }
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    auto cities = Allcity(roadNetwork);
    return helper(roadNetwork, numCities, supplyLocations, cities);
}
