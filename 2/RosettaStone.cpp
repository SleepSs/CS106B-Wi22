...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for (int i = 0; i+kGramLength-1 < str.size(); i++) {
        segments[str.substr(i, kGramLength)]++;
    }
    return segments;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    donom = sqrt(s);
    if (donom == 0) {
        error("Invalid kGram input");
    }
    for (string key: input) {
        output[key] = input[key] / donom;
    }
    return output;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for (Corpus co: corpora) {
        double sc = cosineSimilarityOf(textProfile, co.profile);
        pq.enqueue(co.name, -sc);
    }
    return pq.dequeue();
}
