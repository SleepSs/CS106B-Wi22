...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    flood.resize(rows, cols);
    flood.fill(false);
    for (GridLocation loc: sources) {
        if (terrain[loc] <= height) {
            flood[loc] = true;
            q.enqueue(loc);
        }
    }
    while (!q.isEmpty()) {
        ...
        int row = loc.row;
        int col = loc.col;
        if (flood.inBounds(row + 1, col) && !flood[row+1][col] && terrain[row+1][col] <= height) {
            q.enqueue(GridLocation(row+1, col));
        ...
        if (flood.inBounds(row, col + 1) && !flood[row][col + 1] && terrain[row][col + 1] <= height) {
            q.enqueue(GridLocation(row, col + 1));
