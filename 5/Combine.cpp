...
#include "Combine.h"
using namespace std;
Vector<DataPoint> combine_helper(Vector<DataPoint>& v1, Vector<DataPoint>& v2) {
    int v1s = v1.size();
    int v2s = v2.size();
    int tsize = v1s + v2s;
    int i1 = 0;
    ...
    for(int i = 0; i < tsize; i++) {
        if (i2 >= v2s || (i1 < v1s && v1[i1].weight
                          <= v2[i2].weight)) {
            ans.add(v1[i1]); // Must use add, [] or put will be out of bounds
            i1++;
        } else {
            ans.add(v2[i2]);
            i2++;
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    if (sequences.size() == 0) {
        ...
        auto v1 = sequences[0];
        auto v2 = sequences[1];
        return combine_helper(v1, v2);
    } else {
        auto left = combine(sequences.subList(0, sequences.size() / 2));
        auto right = combine(sequences.subList(sequences.size() / 2));
        return combine_helper(left, right);
