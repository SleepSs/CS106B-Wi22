...
bool conflict(const Set<Shift>& shift1, const Set<Shift>& shift2) {
    for (Shift sh1: shift1) {
        for (Shift sh2: shift2) {
            if (overlapsWith(sh1, sh2)) {
                return true;
            }
        }
    }
...
int ScheduleHelper(Set<Shift> shifts, int maxHours, Set<Shift>& Chosen, int value) {
    if (maxHours == 0 || shifts.isEmpty()) {
        return value;
    } else {
        auto sh = shifts.first();
        if (!conflict({sh}, Chosen) && (maxHours - lengthOf(sh) >= 0)) {
            auto copy = Chosen;
            Chosen.add(sh);
            shifts.remove(sh);
            int v1 = ScheduleHelper(shifts, maxHours - lengthOf(sh), Chosen, value + valueOf(sh));
            int v2 = ScheduleHelper(shifts, maxHours, copy, value);
            if (v2 >= v1) {
                Chosen = copy;
                return v2;
            } else {
            ...
            shifts.remove(sh);
            return ScheduleHelper(shifts, maxHours, Chosen, value);
        }
    }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if (maxHours < 0) {
        ...
        Set<Shift> tmp;
        ScheduleHelper(shifts, maxHours, tmp, 0);
        return tmp;
    }
