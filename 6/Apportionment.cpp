...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if (populations.size() > numSeats) {
        error("Shortage of seats");
    } else {
        HeapPQueue pq;
        Map<string, int> allo;
        for (auto key: populations.keys()) {
            pq.enqueue({key, -populations[key]/sqrt(2)});
            allo[key] = 1;
            numSeats--;
        }
        while (numSeats) {
            auto state = pq.dequeue();
            allo[state.name]++;
            numSeats--;
            pq.enqueue({state.name, -populations[state.name]/sqrt(allo[state.name] * (allo[state.name]+1))});
        }
        return allo;
    }
}
