...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    while(index != 1) {
        if (elems[index/2].weight < data.weight) break;
...
DataPoint HeapPQueue::dequeue() {
    ...
    auto dd = elems[logicalSize];
    logicalSize--;
    int index = 1;
    while(index*2 <= logicalSize) {
        if (elems[index*2].weight < elems[index].weight && (index*2+1 > logicalSize ||
                                                            elems[index*2+1].weight >= elems[index*2].weight)) {
            swap(elems[index*2], elems[index]);
            index *= 2;
        } else if (index*2+1 <= logicalSize && elems[index*2+1].weight < elems[index].weight) {
            swap(elems[index*2+1], elems[index]);
            index = index*2 + 1;
        } else {
            break;
        }
